...
for f in frac:
        ...
        ytr_temp=ytr.reshape(len(ytr))
        new_y_train,fi=flip.nearest_first_label_flip(X, ytr_temp, f,GAMMA,C)
        ...
        X=np.array(X)
        ytr=np.array(ytr).reshape(len(ytr),1)
        fi=np.array(fi).reshape(len(fi),1)
        
        #draw_contours(X, ytr, model, fi, 'SVM - nearest attack', 'nearest_attack')
        ...
        #draw_contours(X, ytr, model, fi, 'LID-SVM - nearest attack', 'lid_svm')
...
plt.title('flip(nearest) vs accuracy')
plt.legend()
plt.show()
